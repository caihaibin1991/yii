...
class CSecurityManager extends CApplicationComponent {
	...
	public function init() {
		...
		if (! extension_loaded ( 'openssl' ) && ! extension_loaded ( 'mcrypt' )) {
			throw new InvalidConfigException ( 'Encryption requires the OpenSSL PHP extension or Mcrypt PHP extension' );
		}
		// ops check
	...
	/**
	 * note:the code is copied from the yii framework
	 *
	 ...
	 * @throws CException if PHP Mcrypt extension is not loaded or key is invalid
	 */
	public function ops_encrypt($data, $key = null) {
		if ($key === null)
			$key = $this->getEncryptionKey ();
		if ($this->cryptAlgorithm != 'rijndael-128') {
			throw new CException ( Yii::t ( 'yii', 'the CSecurityManager is modified by other guy to support php 7.2. currently only support the rijndael-128,if you need other encryption algorithm you need to change the code in here!by the way ,in the yii framework 2 the encryption algorithm only allows using AES-128-CBC ,AES-192-CBC,AES-256-CBC' ) );
		}
		...
		$ops_encrypt_data = @openssl_encrypt ( $data, $this->ops_cipher, $key, false, $iv );
		$encrypted = $iv . base64_decode ( $ops_encrypt_data );
		return $encrypted;
	}
	 ...
	 * @throws CException if PHP Mcrypt extension is not loaded or key is invalid
	 */
	public function ops_decrypt($data, $key = null) {
		if ($key === null)
			$key = $this->getEncryptionKey ();
		if ($this->cryptAlgorithm != 'rijndael-128') {
		...
		$iv = $this->substr ( $data, 0, $ivSize );
		$ops_data = $this->substr ( $data, $ivSize, $this->strlen ( $data ) );
		$decrypted = @openssl_decrypt ( $ops_data, $this->ops_cipher, $key, OPENSSL_RAW_DATA | OPENSSL_NO_PADDING, $iv );
		return $this->ops_pkcs5_unpad ( $decrypted );
	}
	/**
	 ...
	 */
	public function ops_pkcs5_pad($text, $blocksize) {
	 ...
	 * need mbstring module
	 */
	public function ops_pkcs5_unpad($text) {
	...
	protected function validateEncryptionKey($key) {
		if (is_string ( $key )) {
			// ops if the security manager has set EncryptionKey , when yii is initializing this place will be triggered.
			if (! extension_loaded ( 'mcrypt' )) {
				return $this->ops_validateEncryptionKey ( $key );
			}
			// ops end
